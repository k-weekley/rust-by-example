// `fmt::Debug` hardly looks compact and clean, so it is often adventageous to customize the output
// appearance.  This is done by manually implementing `fmt::Display`, which uses the {} print
// marker.  Implementing it looks like this:

// Import (via `use`) the `fmt` module to make it available.
use std::fmt;

// Define a structure for which `fmt::Display` wil be implemented.  This is
...
// `fmt::Display` is not implemented for `Vec<T>` or for any other generic containers.  `fmt::Debug`
...
impl fmt::Display for MinMax {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        // Use `self.number` to refer to each positional data input.
...
fn main() {
    ...
    // Activity
    let my_num = Complex{real: 3.3, imag: 7.2};
    println!("Display: {}", my_num);
    println!("Debug: {:?}", my_num);
}
